...
def dados_paciente(request, id):
    ...
    if not paciente.nutri == request.user:
        messages.add_message(request, constants.ERROR, 'Você não possui permissão para acessar os dados deste paciente!')
...
def plano_alimentar(request, id):
    ...
    if not paciente.nutri == request.user:
        messages.add_message(request, constants.ERROR, 'Você não possui permissão para ver os dados deste paciente!')
        return redirect('/dados_paciente/')

    if request.method == "GET":
        r1 = Refeicao.objects.filter(paciente = paciente).order_by('horario')
        opcoes = Opcao.objects.all()
        return render(request, 'plano_alimentar.html', {'paciente': paciente, 'refeicao': r1, 'opcoes': opcoes})
